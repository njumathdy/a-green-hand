...
def saveMoveInfoToFile(movieId, movieName):
    ...
    try:
        ...
        parseData(movieId, movieName, responseTxt) # 这一步报错，跑到exception块执行，输出"exception occured!"
        # if returnLastId == "-1":
        #     print("===============")
        #     print(movieName)
        #     print(url)
...
def parseData(movieId, movieName, htmlContent):
    ...
    for item in data:
        ...
        saveOriginalDataToDatabase(item["id"], movieId, movieName, item["content"]) # 调用数据库函数
        print("成功导入数据库")
        # lastId = item['id']
    # return lastId
...
def saveOriginalDataToDatabase(msgId, movieId, movieName, originalData):
    ...
    cursor = conn.cursor()
    ins='insert into originalData (id, movieId, originalInfo, movieName) values (%s, %s, %s, %s)'
    # v = (movieId+ "_" + msgId, movieId, originalData, movieName)
    cursor.execute(ins, (movieId+"_"+msgId, movieId, originalData, movieName))
    cursor.close()
    conn.commit()
    conn.close()
    print("存入数据库")
